...
async function runAuth(state: UrbitVisorState, backendShip: string, sendResponse: any) {
  ...
  const decp = ob.patp2dec("~" + backendShip);
  const scry = await airlock.scry({app: "graph-store", path: `/graph/${"~"+airlock.ship}/dm-inbox/node/siblings/newest/lone/1/${decp}`})
...
function bulkRequest(
  ...
  if (existingPerms && request.data.every((el: UrbitVisorAction) => existingPerms.includes(el)))
